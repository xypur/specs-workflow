...
5. 勾选已完成的任务 `- [x]`，并让 `.specs/index.md` 状态总表与任务摘要表保持同步（`draft` → `design` → `implementing` → `implemented` → `archived`）。任务勾选时同步更新 `Progress` 列、索引状态栏与各模块的状态块。
...
- `tasks.md`：层级任务按 `N.M` 编号（与需求编号解耦），按**依赖**划分，并按选定的排序策略组织（Foundation-First / Feature-Slice / Risk-First / Hybrid），每条引用 `_Requirements: x.y_`，可选 / MVP 可跳过的子任务用 `*` 标注，在有意义的里程碑处设置运行测试/构建的 Checkpoint 任务，JSON 形式的 Task Dependency Graph（有序 waves），以及末尾的状态块（`进度 done/total · 当前：<module>.<当前任务> · 门禁链：<阶段>.<末位任务> → …`）。
...
- 不让索引状态栏、`Progress` 列或模块的状态块与任务勾选状态脱节。
