...
1. 编码之前，先创建 `.specs/<module>/`，包含 `requirements.md`、`design.md`、`tasks.md`、`CHANGELOG.md` 四个文件，并填好需求。在 `.specs/index.md` 状态总表中为模块添加一行。
...
5. 勾选已完成的任务 `- [x]`，并让 `.specs/index.md` 状态总表保持同步（`draft` → `design` → `implementing` → `implemented` → `archived`）。
