...
Create or update the `.specs/$ARGUMENTS/tasks.md` document for the module `$ARGUMENTS`. If `$ARGUMENTS` is empty, ask the user for the module name first.
...
Read the module's `.specs/$ARGUMENTS/requirements.md` first so every requirement clause referenced by `_Requirements:` actually exists — no dangling references. If `.specs/$ARGUMENTS/tasks.md` already exists, preserve its content and extend it rather than rewriting.
