...
Create or update the `.specs/$1/requirements.md` document for the module `$1`. If `$1` is empty, ask the user for the module name first.
...
- **Introduction**: 1–2 paragraphs — what this module is, what problem it solves, and who uses it. Lead with the core technical pain point.
- **Glossary**: define every key term the document relies on (components, state, protocols, config names). One precise bullet per term.
...
If `.specs/$1/requirements.md` already exists, preserve its content and extend it rather than rewriting. If `.specs/index.md` exists and the module `$1` is not listed in its status table, add a row for it.
