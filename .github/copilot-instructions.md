...
5. Check off completed tasks `- [x]` and keep the `.specs/index.md` status and Task Summary tables in sync (`draft` → `design` → `implementing` → `implemented` → `archived`). The index is the single source of task status; derive its `Progress` column, status bar, next task, and next gate from the task checkboxes and dependencies.
