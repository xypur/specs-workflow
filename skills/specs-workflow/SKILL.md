...
If `.specs/` already exists, read `.specs/index.md` first to learn the module status table, dependencies, and execution order. Determine whether the requested work belongs to an existing module or needs a new one. If `.specs/` does not exist, propose creating it before starting the task.

### Step 2: Create Module Documents Before Coding

Before writing any code for a new module or feature, create `.specs/<module>/` containing `requirements.md`, `design.md`, `tasks.md`, and `CHANGELOG.md`, and fill in the requirements. Add a row for the module in the `.specs/index.md` status table. Use the skeletons in [references/file-templates.md](references/file-templates.md) for structure, the per-section prompting guidance in [references/prompt-templates.md](references/prompt-templates.md) for depth, the quality gates in [references/checklists.md](references/checklists.md) for acceptance, and the [references/examples/traceability.md](references/examples/traceability.md) example for the exact traceability format.
...
Execute tasks in dependency order. Check off completed tasks with `- [x]` and keep the `.specs/index.md` module status table up to date (move the module along `draft → design → implementing → implemented`). At each checkpoint, run the tests/build and report results.
...
```
.specs/
├── index.md             # Global index + module status table + dependencies/priority
└── <module>/
...
| 5 | Check off tasks `- [x]` and sync the `.specs/index.md` status table as you go |
...
- Do not leave completed tasks unchecked or out of sync with the `.specs/index.md` status table.
